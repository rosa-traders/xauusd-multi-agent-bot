# ==========================================
# Added the new agent as LevelSweepAgent in multi_agent_xauusd_v6.py. 
#Dynamic Risk Allocation (A/B Grading): Not all setups are created equal. 
#Dropping the risk to 0.5% for a B-grade setup (e.g., a sweep with lower volume or weak H4 alignment) while keeping 1.0% for an A-grade setup protects your capital during choppy conditions while maximizing RR during high-probability manipulations.
#
#The M15/M5 Dual-Timeframe Flow: Having the M15 arm the setup (the sweep) and the M5 act as the trigger (FVG or CHoCH) perfectly mirrors how discretionary smart-money traders operate. It prevents the bot from catching falling knives.
#
#PDH/PDL & London Extremes: Tracking the Previous Day High/Low and London High/Low is the exact right approach for gold. These are the exact pools of liquidity that bullion banks target to fill large orders.
#
#Setup Funnel Telemetry: Adding the shared_state.funnel counters to the dashboard is a massive quality-of-life upgrade. If the bot goes a week without trading, you will no longer have to guess why; the dashboard will explicitly tell you if setups are dying because of "counter_bias", "risk_too_wide", or "failed_triggers".
# ==========================================#
import logging
import math
import os
...
MAGIC_SWING = 999998
MAGIC_LEVEL = 999997
SYSTEM_MAGICS = {MAGIC_MICRO, MAGIC_SWING, MAGIC_LEVEL}
...
# The MetaTrader5 Python package does NOT export SYMBOL_FILLING_* constants. These are the
# documented bit flags of symbol_info().filling_mode.
SYMBOL_FILLING_FOK = 1
SYMBOL_FILLING_IOC = 2

# Everything the bot needs from the mt5 module; verified at startup so a missing name
# fails immediately instead of at the first order.
REQUIRED_MT5_NAMES = (
...
class TradingState:
    def __init__(self):
        ...
        self.levels = {}          # reference levels tracked by LevelSweepAgent
        self.level_setup = None   # currently armed level-sweep setup (or None)
...
# ==========================================
# 5.5 LEVEL SWEEP AGENT (PDH/PDL + London H/L, confluence-scored)
# ==========================================
class LevelSweepAgent(ExecutionAgent):
    """Sweep-and-reclaim of previous-day and London highs/lows.

    Flow (one agent, two timeframes):
      M15  detect a sweep of a reference level on the last CLOSED bar (wick beyond the
           level, close back on the right side) and score the confluence.
      M5   once armed, enter on the first trigger after the sweep candle closed:
             - FVG in the reclaim direction, or
             - CHoCH/BOS (close beyond the minor structure that preceded the extreme).
      SL   beyond the sweep extreme (structural), TP = rr x risk.
      Size graded by score: A-grade -> risk_a_pct, B-grade -> risk_b_pct, below min -> no trade.

    Score (max 10):  sweep +1 | level stacking up to +2 | volume +1/+2 | rejection candle +1
                     | H4 zone touched (bias-aligned) +2 | H4 bias aligned +1 | killzone hour +1
    The thresholds are placeholders: tune them from the logged score distribution
    and a replay backtest, not from this docstring.
    """

    LONDON_HOURS = (7, 12)                 # UTC, end exclusive (fixed hours: mind US/UK DST)
    KILLZONES = ((7, 10), (12, 15))        # inclusive UTC hours
    LOW_LEVELS = ("PDL", "LDNL")           # swept from below -> bullish reclaim
    HIGH_LEVELS = ("PDH", "LDNH")          # swept from above -> bearish reclaim
    HTF_REFRESH_SEC = 600
    ...
    def _refresh_htf(self):
        ...
        self._htf = htf
        self._htf_ts = now if htf else now - self.HTF_REFRESH_SEC + 30   # retry soon if empty
    ...
    def _london_levels(self, df, offset):
        ...
        # Only a COMPLETED London session is a liquidity pool for NY
        if int(hour.iloc[-1]) >= self.LONDON_HOURS[1] and len(sess) >= 16:
    ...
    def _detect(self, df, levels, bias):
        ...
        # primary = the level pierced the deepest
        ...
        if not aligned and not self.allow_counter_bias:
        ...
        # level stacking (other reference levels or a $50 round number within 0.5 ATR)
        tol = 0.5 * atr
        cands = [v for k, v in levels.items() if k != name]
        cands.append(round(level / 50.0) * 50.0)
        ...
        if score >= self.a_score:
            grade, risk = "A", self.risk_a_pct
        elif score >= self.min_score:
            grade, risk = "B", self.risk_b_pct
        else:
            grade, risk = None, 0.0
    ...
    def step(self):
        ...
        # expire an old armed setup
        if self.pending is not None and time.time() > self.pending['expires_at']:
            logger.info(f"[{self.name}] Armed setup expired without a trigger.")
            shared_state.count("lvl_setup_expired")
            self._disarm()

        # ---- M15: levels + sweep detection on each new closed bar ----
        self._refresh_htf()
...
if __name__ == "__main__":
    ...
    if missing:
        logger.error(f"The installed MetaTrader5 package is missing: {missing}. "
                     f"Upgrade it with: pip install --upgrade MetaTrader5")
        quit()
    ...
    agents = [macro, killzone, swing, micro, levelsweep, manager, telemetry]
    for a in agents:
        a.start()
